...
export function NewProductForm({ categories }: Props) {
  ...
  async function generateAI() {
    ...
    try {
      ...
      const data = await res.json() as { description?: string; short_description?: string; instagram_caption?: string }
    ...
    try {
      const res = await fetch('/api/products', {
        ...
        body: JSON.stringify({
          ...
          images, tags, description, short_description: shortDescription, ai_description: instagramCaption,
        }),
      })
  ...
  return (
    <main className="flex-1 p-8 max-w-5xl">
      <div className="flex items-center justify-between mb-8">
        ...
        <button onClick={() => router.back()} className="text-sm text-[#888] hover:text-[#1a1a1a]">← Back</button>
      </div>
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="bg-white border border-[#ede9e3] rounded-lg p-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input {...register('material')} className="input" placeholder="Cotton, Silk..." />
                </div>
                <div>
                ...
                <div className="col-span-2">
                  ...
                  <input {...register('care_instructions')} className="input" placeholder="Dry clean only" />
                </div>
              </div>
            </div>
            ...
            <div className="bg-white border border-[#ede9e3] rounded-lg p-6">
              ...
              <input value={tagInput} onChange={e => setTagInput(e.target.value)} onKeyDown={addTag}
                className="input text-sm" placeholder="Type tag and press Enter (e.g. floral, festive)" />
            </div>
            ...
            <div className="bg-white border border-[#ede9e3] rounded-lg p-6">
              <div className="flex justify-between items-center mb-4">
                ...
                <button type="button" onClick={() => { void generateAI() }} disabled={aiLoading}
              ...
              <div className="space-y-3">
                <div>
                  ...
                  <input value={shortDescription} onChange={e => setShortDescription(e.target.value)}
                    className="input" placeholder="One compelling sentence" />
                </div>
                <div>
                  <label className="label">Full Description *</label>
                  <textarea value={description} onChange={e => setDescription(e.target.value)}
                    className="input resize-none" rows={6} placeholder="Detailed product description..." />
                </div>
                {instagramCaption && (
                  <div>
              ...
              <div className="grid grid-cols-2 gap-2 mb-3">
                {images.map((url, i) => (
                  <div key={url} className="relative aspect-[3/4] bg-[#f5f0e8] rounded overflow-hidden">
                    ...
                    {i === 0 && <span className="absolute top-1 left-1 text-[10px] bg-[#1a1a1a] text-white px-1.5 py-0.5 rounded">Main</span>}
                ...
                <label className="aspect-[3/4] border-2 border-dashed border-[#ede9e3] rounded flex flex-col items-center justify-center cursor-pointer hover:border-[#1a1a1a] transition-colors">
                  {uploadLoading
                    ...
                    : <><Upload size={20} className="text-[#bbb]" /><span className="text-xs text-[#bbb] mt-1">Upload</span></>
                  }
                  <input type="file" accept="image/*" className="hidden"
                    onChange={e => { if (e.target.files?.[0]) void uploadImage(e.target.files[0]) }} />
                </label>
              </div>
              <p className="text-xs text-[#999]">First image is the main image. Max 5MB.</p>
            </div>
            ...
            <div className="bg-white border border-[#ede9e3] rounded-lg p-6">
              ...
              <div className="space-y-3">
                <label className="flex items-center gap-3 cursor-pointer">
                  ...
                  <div><p className="text-sm font-medium">Active</p><p className="text-xs text-[#999]">Visible in store</p></div>
                </label>
                <label className="flex items-center gap-3 cursor-pointer">
                  <input type="checkbox" {...register('is_featured')} className="accent-[#1a1a1a]" />
                  <div><p className="text-sm font-medium">Featured</p><p className="text-xs text-[#999]">Show on homepage</p></div>
                </label>
              </div>
            </div>
